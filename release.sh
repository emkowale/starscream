#!/usr/bin/env bash
# Starscream theme release script.
set -euo pipefail

# ==== CONFIG ==================================================================
OWNER="emkowale"
REPO="starscream"
THEME_SLUG="starscream"      # top-level folder inside release zip
...
if [[ "$GIT_OK" -eq 1 ]]; then
  ...
  git push origin main || warn "Could not push main"
  git push origin "v${NEXT}" || warn "Could not push tag v${NEXT}"
