...
add_action('admin_menu', function () {
  add_theme_page(
    ...
    'Page Builder Guide',
    'edit_theme_options',
    'starscream-page-builder-guide',
    'starscream_render_page_builder_guide'
  );
});

add_action('admin_enqueue_scripts', function ($hook_suffix) {
  if ($hook_suffix !== 'appearance_page_starscream-page-builder-guide') return;
  ...
  $enqueue_style = static function ($handle, $relpath, $deps = []) use ($theme_version) {
    ...
    wp_enqueue_style($handle, starscream_asset_uri($relpath), $deps, $version);
  };

  $enqueue_script = static function ($handle, $relpath, $deps = []) use ($theme_version) {
    $path = starscream_locate($relpath);
    if (!$path || !file_exists($path)) return;
    ...
    wp_enqueue_script($handle, starscream_asset_uri($relpath), $deps, $version, true);
  };

  $enqueue_style('starscream-guide-site', 'assets/css/site.css');
  $enqueue_style('starscream-guide-pages', 'assets/css/pages.css', ['starscream-guide-site']);
  $enqueue_style('starscream-guide-admin', 'assets/css/admin-page-builder-guide.css', ['starscream-guide-pages']);

  $enqueue_script('starscream-guide-admin', 'assets/js/admin-page-builder-guide.js');
});
